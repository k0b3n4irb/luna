...
| SA-1 | **B** | `conflict()` bus contention, faithful HV timer, per-access cycle cost; residual = batched (non-cothread) scheduler grain, not a value/feature bug. **Regraded 2026-09-11:** the SA-1 CPU never received IRQ/NMI (fixed: CIV/CNV delivery); still **open with possible game impact:** CC1/CC2 character-conversion formats, BW-RAM bitmap view, RDYB wait, per-device DMA, side-split registers ([`luna_sa1_gaps.md`](luna_sa1_gaps.md) #7-#15) | [`luna_sa1_gaps.md`](luna_sa1_gaps.md), [`sa1_status.md`](archive/sa1_status.md) | **2026-09-11** |
