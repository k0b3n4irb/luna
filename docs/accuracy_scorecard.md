...
| Controllers | **B+** | Standard pad (auto-read `$4218-$421B` + serial `$4016/$4017`, idle-high data line after the 16 shifts), SNES Mouse and Super Scope (ares `controller/mouse`, `controller/super-scope` ports; PPU H/V latch through WRIO; the Super Scope's held-button turbo / trigger-lock state machine is deliberately simplified to per-frame scripted states), selectable per port, scriptable from CLI / MCP / GUI. **Not emulated:** multitap (`$421C-$421F` read a hard `0`, `cpu_regs.rs`; no 4-pad serial multiplexing) and the Justifier — 3-5-player and Justifier titles are limited to what two plain ports give. No differential oracle for the serial devices (unit tests + in-game validation only) | `luna-core/src/controller.rs`, `luna-core/src/cpu_regs.rs` | **2026-09-18** |
