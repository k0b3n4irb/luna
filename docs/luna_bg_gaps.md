...
| ~~17~~ | ~~Overscan picture height~~ | `main.cpp:4`, `io.cpp:641` | ✅ **DONE 2026-09-14**: the frame's picture height is latched at line 0 from SETINI bit 2 (ares `state.overscan`, Mesen2 `_overscanFrame`) — 224 rows, or 239 with overscan. The persistent framebuffer holds 239 rows and `Ppu::frame_height` / `Emulator::frame_height` say how many the frame uses; every consumer (PNG, RGBA, fbhash, the native capture, the golden harness, the GUI canvas + egui crop, screenshots) follows it. A non-overscan game is byte-identical to before (41-title fbhash A/B). |
