...
| 17 | Overscan picture height | `main.cpp:4`, `io.cpp:641` | **timed, not displayed** (2026-09-12): `vdisp` now follows SETINI bit 2 in both regions (225 / 240), so NMI, HVBJOY, HDMA and the VRAM/OAM access gate land on the hardware line. The framebuffer still stores 224 rows, so a game running in 239-line overscan keeps its last 15 picture lines off-screen. Needs a variable frame height across `luna-api`, the GUI, the goldens and the screenshot path — a feature, not a bug fix. |
