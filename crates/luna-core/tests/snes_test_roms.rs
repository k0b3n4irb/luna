...
ppu_test!(
    ...
    ignore = "HiColor chart (gap #7): PINNED 2026-06-22 vs the shipped hardware reference PNG. Technique (.asm): H-IRQ at HTIME=190 each line; the handler DMAs 8 colours into CGRAM in HBlank (h≈299) and resets CGADD every 8 lines, building a 64-colour palette per 8-line tile-row. Diff vs reference: 81% exact, and the WRONG pixels are exactly the FIRST line of each 8-line band (y=0,8,16,…) — i.e. an OFF-BY-ONE: the first line of each tile-row renders with the PREVIOUS band's palette. Root: luna renders line N with its END-of-line CGRAM (after line N's HBlank DMA) via `sched_one_line`, instead of the pre-HBlank-DMA state. TWO surgical fixes were tried against the reference per-row diff and BOTH failed: (1) a flush-before-DMA on the mid-line MDMA → no-op (the DMAs land in HBlank, dot clamps to 256); (2) excluding $2121/CGADD from the partial-flush so the first band-line renders at line-end with the post-DMA palette → it then rendered band-1 instead of band-0, but the diff did NOT improve and slightly worsened (y=24 newly wrong). So it is NOT a simple per-line palette swap: the per-line palette is built INCREMENTALLY (8 colours/line, CGADD reset every 8) and the PPU reads CGRAM per dot, so a per-line model can't reproduce it. Also the bottom smooth-gradient band (y≈166-221, ~56 rows) is a separate, larger slice of the 19% diff. RULED OUT (2026-06-22): the H-IRQ handler reads the V-counter ($213D OPVCT) to index which tile-row palette to DMA, but luna returns the CORRECT scanline (108=$6C, 109=$6D, …) and DOES fire the H-IRQ at HTIME=190 (the scorecard's 'H-IRQ ignores HTIME' is stale) — so it is NOT an IRQ/OPVCT-timing bug. The crux is the render-vs-HBlank-CGRAM-DMA flush interaction itself (luna renders line N with line N's end-of-line CGRAM). Multiple hypotheses exhausted; deferred to a focused session with the reference per-row diff as the live oracle."
);
ppu_test!(
    ppu_hdma_hicolor128,
